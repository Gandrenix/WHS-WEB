...
import { AdminSidebar, MusicManager } from '@/features/admin-dashboard';
...
export default async function MusicAdminPage() {
  ...
  const [projects, commentCount, songs] = await Promise.all([
    getAllProjects(),
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 lg:p-10 max-w-6xl mx-auto w-full">
          <div className="mb-10 pb-6 border-b border-white/15">
            <h1 className="text-3xl font-black text-white uppercase tracking-tight">Música — Playlist del Header</h1>
            <p className="text-[#F2EDE4]/70 text-xs mt-1">
              Administra las canciones del reproductor junto al logo: sube MP3, ordénalas y elige cuál suena por
              defecto al primer clic (&ldquo;PRINCIPAL&rdquo;).
            </p>
          </div>

          <MusicManager songs={songs} />
        </div>
      </main>
    </div>
  );
